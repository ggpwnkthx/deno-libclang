...
/**
 * Check if a cursor refers to a static function
 *
 * @param cursor - CXCursor or Uint8Array buffer for a function
 * @returns True if the function is static, false otherwise
 */
export function isStaticFunction(cursor: CXCursor | Uint8Array): boolean {
  const sym = getSymbols();
  const cursorArg = cursor instanceof Uint8Array
    ? cursor as unknown as CXCursor
    : cursor;
  return sym.clang_Cursor_isStaticFunction(cursorArg) !== 0;
}

/**
 * Check if a cursor refers to an inline function
 *
 * @param cursor - CXCursor or Uint8Array buffer for a function
 * @returns True if the function is inline, false otherwise
 */
export function isInlineFunction(cursor: CXCursor | Uint8Array): boolean {
  const sym = getSymbols();
  const cursorArg = cursor instanceof Uint8Array
    ? cursor as unknown as CXCursor
    : cursor;
  return sym.clang_Cursor_isInline(cursorArg) !== 0;
}

/**
 * Check if a cursor refers to a variadic function
 *
 * @param cursor - CXCursor or Uint8Array buffer for a function
 * @returns True if the function takes variadic arguments, false otherwise
 */
export function isVariadicFunction(cursor: CXCursor | Uint8Array): boolean {
  const sym = getSymbols();
  const cursorArg = cursor instanceof Uint8Array
    ? cursor as unknown as CXCursor
    : cursor;
  return sym.clang_Cursor_isVariadic(cursorArg) !== 0;
}
